...
test("No.020追加修正: 4テーマを指定色で一元管理する", () => {
  ...
  assert.equal(SNS_CARD_THEMES.cream.background, "#F2E9D8");
  assert.equal(SNS_CARD_THEMES.mint.accent, "#146A65");
  assert.equal(SNS_CARD_THEMES.sky.text, "#172D40");
  assert.equal(SNS_CARD_THEMES.sand.accent, "#6F5728");
...
test("No.020追加修正: Canvasプレビュー、テーマUI、設定分離、0.20.1を備える", async () => {
  ...
  assert.match(html, /Version 0\.23\.1/);
