...
test("No.020追加修正: 4テーマを指定色で一元管理する", () => {
  ...
  assert.equal(SNS_CARD_THEMES.cream.background, "#F7F3EA");
  assert.equal(SNS_CARD_THEMES.mint.photoFrame, "#CFE3DD");
...
test("No.020追加修正: ワードマークとOtomoの座標を集約する", () => {
  ...
  assert.deepEqual(SNS_CARD_LAYOUT.character, { x: 580, y: 900, width: 400 });
});
...
test("No.020追加修正: Canvasプレビュー、テーマUI、設定分離、0.20.1を備える", async () => {
  ...
  assert.match(html, /Version 0\.20\.1/);
